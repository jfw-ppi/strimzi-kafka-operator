...
IMPORTANT: Starting from Strimzi 0.50.0, standalone Topic Operator deployments require a volume mount for cluster CA certificates at `/etc/cluster-ca-certs/`.
...
If you are running a standalone Topic Operator deployment, you must manually configure this volume mount before upgrading to version 0.50.0 or later.
Without this configuration, the Topic Operator will fail to start with a `java.io.FileNotFoundException: "/etc/cluster-ca-certs/*.crt"` error.
This requirement applies regardless of whether Cruise Control is enabled.
