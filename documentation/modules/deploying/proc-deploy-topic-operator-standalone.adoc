...
This volume mount must contain the cluster CA certificate file (`ca.crt`) that the Topic Operator uses to establish secure TLS connections to Kafka brokers and Cruise Control.
...
+
Starting from Strimzi 0.50.0, the Topic Operator requires access to the cluster CA certificate for establishing secure connections.
