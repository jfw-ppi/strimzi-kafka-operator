...
+
IMPORTANT: Starting from Strimzi 0.50.0, the Topic Operator requires a volume mount for cluster CA certificates at `/etc/cluster-ca-certs/` when Cruise Control is enabled with SSL.
This volume mount must contain the cluster CA certificate file (`ca.crt`) that the Topic Operator uses for secure TLS connections to Cruise Control.
If you are not using Cruise Control or if SSL is not enabled for Cruise Control, this volume mount can be omitted.
The deployment file includes this configuration by default for consistency with Cluster Operator deployments, but you should configure it based on your actual Cruise Control SSL requirements.
+
...
. Configure the cluster CA certificate volume mount for Cruise Control SSL connections.
+
Starting from Strimzi 0.50.0, when Cruise Control is enabled with SSL, the Topic Operator requires access to the cluster CA certificate for secure TLS connections to Cruise Control.
The Topic Operator expects to find the CA certificate at `/etc/cluster-ca-certs/ca.crt`.
+
**Important:** This certificate is specifically for Cruise Control SSL connections. It is NOT used for Kafka broker connections.
The certificate file is only accessed when both Cruise Control and SSL are enabled for Cruise Control.
+
**Note for Cluster Operator users:** If you configure Cruise Control in your `Kafka` custom resource (the `.spec.cruiseControl` section exists), the Cluster Operator automatically enables SSL for Cruise Control and creates the required volume mount.
+
If you are deploying standalone and are not using Cruise Control or if SSL is not enabled, you can skip this configuration step.
+
If you need the certificate for Cruise Control SSL, create a Kubernetes Secret containing the cluster CA certificate and mount it in the Topic Operator deployment:
+
.Example cluster CA certificate volume configuration
...
type: Opaque
data:
  ca.crt: __BASE64_ENCODED_CA_CERTIFICATE__ # <1>
----
<1> The cluster CA certificate in base64-encoded PEM format.
+
Then ensure the deployment YAML includes the volume and volume mount:
+
.Example volume mount configuration in deployment
[source,yaml,subs=+quotes]
----
spec:
  template:
    spec:
      volumes:
        - name: cluster-ca-certs
          secret:
            secretName: my-cluster-cluster-ca-cert # <1>
      containers:
        - name: strimzi-topic-operator
          volumeMounts:
            - name: cluster-ca-certs
              mountPath: /etc/cluster-ca-certs # <2>
----
<1> The name of the Secret containing the cluster CA certificate.
<2> The mount path must be `/etc/cluster-ca-certs` as this is where the Topic Operator expects to find the certificate when Cruise Control SSL is enabled.
+
NOTE: When deploying the Topic Operator through the Cluster Operator (using a `Kafka` custom resource), this volume mount is automatically created for consistency, regardless of whether Cruise Control is enabled. For standalone deployments, include this mount only if you are using Cruise Control with SSL enabled.
