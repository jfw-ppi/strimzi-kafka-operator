...
+
IMPORTANT: Starting from Strimzi 0.50.0, the Topic Operator requires a volume mount for cluster CA certificates at `/etc/cluster-ca-certs/`.
This volume mount must contain the cluster CA certificate file (`ca.crt`) that the Topic Operator uses for TLS verification when connecting to Kafka brokers.
This requirement applies to all deployments, regardless of whether Cruise Control is enabled.
The deployment file includes this configuration by default, but you need to ensure that the referenced secret exists and contains the correct CA certificate.
+
...
. Configure the cluster CA certificate volume mount for TLS connections.
+
Starting from Strimzi 0.50.0, the Topic Operator requires access to the cluster CA certificate for TLS verification when connecting to Kafka brokers.
The Topic Operator expects to find the CA certificate at `/etc/cluster-ca-certs/ca.crt`.
This requirement applies to all deployments, regardless of whether Cruise Control is enabled or not.
Without this volume mount, the Topic Operator will fail to start with a `java.io.FileNotFoundException: "/etc/cluster-ca-certs/*.crt"` error.
+
You need to create a Kubernetes Secret containing the cluster CA certificate and mount it in the Topic Operator deployment:
+
...
<2> The mount path must be `/etc/cluster-ca-certs` as this is where the Topic Operator expects to find the certificate.
+
NOTE: When deploying the Topic Operator through the Cluster Operator (using a `Kafka` custom resource), this volume mount is automatically created, and you do not need to configure it manually.
